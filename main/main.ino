...
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----)EOF";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) != MQTT_TOPIC_COMANDO) return;
  ...
  String dir = doc.containsKey("dir") ? String((const char*)doc["dir"]) : "stop";
  int speed = doc.containsKey("speed") ? doc["speed"].as<int>() : 200;
  int duration = doc.containsKey("duration") ? doc["duration"].as<int>() : 1000;
  
  velocidad = constrain(speed, 0, 255);
  duration = constrain(duration, 100, 5000);
  stopFlag = false;
  
  Serial.printf("🎮 MQTT: %s | Vel: %d | Dur: %d ms\n", dir.c_str(), velocidad, duration);
  
  if (dir == "forward") adelante();
  else if (dir == "backward") atras();
  else if (dir == "left") izquierda();
  else if (dir == "right") derecha();
  else if (dir == "stop") parar();
  else parar();
  
  if (client.connected()) {
    DynamicJsonDocument respuesta(256);
    respuesta["dir"] = dir;
    respuesta["speed"] = velocidad;
    respuesta["duration"] = duration;
    respuesta["distance"] = ultimaDistancia;
    
    String jsonString;
    serializeJson(respuesta, jsonString);
    client.publish(MQTT_TOPIC_MOVIMIENTO, jsonString.c_str());
  }
  
  unsigned long inicio = millis();
  while (millis() - inicio < (unsigned long)duration && !stopFlag) {
    client.loop();
    delay(10);
  }
  parar();
...
<style>
  ...
  .left, .right { background: #f1c40f; } 
  .stop { background: #c0392b; width: 90px; height: 90px; font-size: 20px; }
  ...
  .distance-container { 
    width: 80%; margin: 20px auto; padding: 15px; background: white; 
  ...
  .distance-text { font-size: 24px; font-weight: bold; margin: 10px 0; }
  .distance-warning { color: #e74c3c; font-weight: bold; display: none; }
  ...
  .camera-input-group input {
    flex: 1; min-width: 200px; padding: 10px; border: 2px solid #3498db;
  ...
  .camera-input-group button:hover { background: #2980b9; }
  .camera-input-group button.clear-btn { background: #e74c3c; }
  .camera-input-group button.clear-btn:hover { background: #c0392b; }
  #cameraFrame {
    width: 100%; height: 700px; border: 2px solid #ddd;
    border-radius: 5px; display: none; background: #000;
  }
  #cameraImg {
    width: 100%; max-height: 700px; border: 2px solid #ddd;
    border-radius: 5px; display: none; background: #000; object-fit: contain;
  }
  .camera-help { font-size: 12px; color: #7f8c8d; margin-top: 10px; }
  .camera-status { margin: 10px 0; font-weight: bold; }
  .camera-status.active { color: #2ecc71; }
  .camera-status.inactive { color: #95a5a6; }
  .camera-controls {
    display: flex; gap: 10px; margin: 15px 0; flex-wrap: wrap;
    justify-content: center; align-items: center;
  }
  .zoom-slider { flex: 1; min-width: 200px; max-width: 400px; }
  .zoom-btn {
    padding: 10px 15px; background: #9b59b6; color: white; border: none;
    border-radius: 5px; cursor: pointer; font-size: 16px; font-weight: bold;
  }
  .zoom-btn:hover { background: #8e44ad; }
  .zoom-info { font-weight: bold; color: #34495e; min-width: 80px; }
</style>
</head>
<body>
...
<div class="slider-container">
  ...
  <input type="range" id="speed" min="0" max="255" value="200" oninput="document.getElementById('speedValue').textContent=this.value">
</div>

<div class="slider-container">
  <label>Duración (ms): </label>
  <input type="range" id="duration" min="100" max="5000" value="1000" step="100" oninput="document.getElementById('durValue').textContent=this.value">
...
function sendCommand(direction) {
  ...
  fetch(`http://${esp32_ip}/move?direction=${direction}&speed=${speed}&duration=${duration}`)
    .then(res => res.json())
    .then(data => {
      document.getElementById("status").textContent = `Comando: ${direction}`;
...
async function checkMQTT() {
  try {
    ...
    if (data.online) { 
      el.textContent = "MQTT: Conectado ✅"; 
      el.className = "online"; 
    } else { 
      el.textContent = "MQTT: Desconectado ❌"; 
...
void handleMove() {
  ...
  Serial.printf("🌐 HTTP: %s | Vel: %d | Dur: %d ms\n", direction.c_str(), velocidad, duration);
  ...
  if (client.connected()) {
    ...
    payload["speed"] = speed;
    payload["distance"] = ultimaDistancia;
    payload["source"] = "http";
    
    String jsonStr;
    serializeJson(payload, jsonStr);
    client.publish(MQTT_TOPIC_MOVIMIENTO, jsonStr.c_str());
  }

  server.send(200, "application/json", "{\"status\":\"ok\",\"cmd\":\"" + direction + "\"}");

  unsigned long start = millis();
  while (millis() - start < (unsigned long)duration && !stopFlag) {
    server.handleClient();
    client.loop();
    delay(10);
  }
...
void conectarMQTT() {
  ...
  if (client.connect(MQTT_CLIENT_ID)) {
    ...
    client.publish("esp32/status", statusJson.c_str(), true);
    
    client.subscribe(MQTT_TOPIC_COMANDO);
    Serial.printf("📬 Suscrito: %s\n", MQTT_TOPIC_COMANDO);
...
void setup() {
  ...
  Serial.println("🛑 Botón de emergencia: Pin 4");
  
  pinMode(TRIG_PIN, OUTPUT);
  pinMode(ECHO_PIN, INPUT);
  Serial.println("📏 Sensor ultrasónico: OK");
...
const unsigned long intervaloReconexion = 30000;
const unsigned long intervaloHeartbeat = 10000;
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (ahora - ultimaReconexion >= intervaloReconexion) {
  ...
  if (client.connected() && ahora - ultimoHeartbeat >= intervaloHeartbeat) {
    ultimoHeartbeat = ahora;
    
    DynamicJsonDocument heartbeat(256);
    heartbeat["alive"] = true;
    heartbeat["uptime"] = millis() / 1000;
    heartbeat["distance"] = ultimaDistancia;
    
    String hbJson;
    serializeJson(heartbeat, hbJson);
    client.publish("esp32/heartbeat", hbJson.c_str());
  }
  ...
  if (ahora - ultimaLectura >= intervaloLectura) {
    ...
    if (distancia > 0 && distancia < 400) {
      ...
      if (client.connected()) {
        DynamicJsonDocument distDoc(128);
        distDoc["distance"] = round(distancia * 10) / 10.0;
        
        String distJson;
        serializeJson(distDoc, distJson);
        client.publish(MQTT_TOPIC_DISTANCIA, distJson.c_str());
      }
    }
  }
